...
class Database:
        ...
        cur.execute(
            ...
            INSERT INTO practice_records
                (record_date, module_id, topic_id, total_questions,
                 correct_questions, duration_seconds, note, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                record_date,
    ...
    # ------------------------------------------------------------ 备考目标
    def set_exam_plan(self, exam_name: str, exam_date: str, target_score: float) -> None:
        """新增或更新备考目标。"""
        self.conn.execute(
            "INSERT INTO exam_plans (exam_name, exam_date, target_score, created_at) "
            "VALUES (?, ?, ?, ?) "
            "ON CONFLICT (exam_name) DO UPDATE SET "
            "exam_date = excluded.exam_date, target_score = excluded.target_score",
            (exam_name, exam_date, target_score, now_str()),
        )
        self.conn.commit()
